...
class TrainTest(parameterized.TestCase):
    ...
    def test_create_model(self):
        ...
        model = train_non_batchnorm.create_model(
            model_cls=models.ConvNeXt_T, num_classes=10, half_precision=False
        )
        params = train_non_batchnorm.initialized(random.PRNGKey(0), 224, model)
        variables = {"params": params}
        x = random.normal(random.PRNGKey(1), (8, 224, 224, 3))
        y = model.apply(variables, x, train=False)
        self.assertEqual(y.shape, (8, 10))

    @parameterized.product(model=("ConvNeXt_T",))
    def test_train_and_evaluate(self, model):
        ...
        with tfds.testing.mock_data(
            ...
            train_non_batchnorm.train_and_evaluate(workdir=workdir, config=config)
